...
description: >
  Architects, generates, and validates Agent Skills following the canonical/skills
  specification. Enforces frontmatter requirements (name, description, license, and
  metadata fields version, author, tags), directory structure conventions, naming
  standards, and content best practices. Guides through a 6-step workflow from
  planning to testing.
  WHEN: create a skill, write a new skill, add a skill, update an existing skill,
  modify a skill, build a skill, author a skill, skill creation, skill development,
  design a skill, new agent capability, generate skill file.
license: Apache-2.0
metadata:
...
- ✅ Auto-detect the skills directory (checks `.github/skills/`, then `skills/`, then CWD)
...
- ✅ SKILL.md exists
- ✅ YAML frontmatter has required fields (`name`, `description`, `version`, `author`, `license`)
- ✅ `name` in YAML matches directory name
- ✅ `version` follows SemVer format (X.Y.Z)
- ✅ `description` contains a `WHEN:` trigger clause
- ⚠️  Advisory: `tags` field present (recommended)
- ⚠️  Advisory: presence of `references/` and `scripts/` directories
