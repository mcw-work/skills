...
### YAML Frontmatter
Follows the [agentskills.io specification](https://agentskills.io/specification#metadata-field):
`author`, `version`, and `tags` live under the `metadata:` key; `license` is top-level.
* **Name:** Max 64 chars, lowercase/numbers/hyphens only.
* **Description:** Max 1024 chars. Must include a `WHEN:` clause with 8+ comma-separated trigger phrases. Minimum 20 words before the WHEN clause.
...
* [ ] **Testing:** Tested with multiple capability tiers of your AI agent (e.g., fast/standard/premium models).
